...
const Card = ({data, initialBookmarked, handleBookmark}) => {
  ...
  return (
    <div className='h-[140px] w-[240px] md:h-[240px] md:w-[470px] relative rounded-lg overflow-clip'>
      <div
      ...
      <div className='absolute w-full h-[70xp] md:h-[100px] bottom-0 p-6 bg-gradient-to-t from-black/75 to-black/0'>
        <div className='flex gap-2 items-center text-gray-300 h-[19px]'>
          <p className='text-xs md:typographyBML'>{data.year}</p>
          <GoDotFill className='h-[3px] w-[3px]' />
          <div className='flex gap-[6px] items-center'>
            <RiFilmFill className='w-3 h-3' />
            <p className='text-xs md:typographyBML'>{data.presentation}</p>
          </div>
          <GoDotFill className='h-[3px] w-[3px]' />
          <p className='text-xs md:typographyBML'>{data.ageRestriction}</p>
        </div>
        <div>
          <h1 className='text-sm md:typographyHSM'>{data.title}</h1>
        </div>
      </div>
